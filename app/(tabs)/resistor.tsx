...
export default function Resistor() {
  ...
  const [scaleAnim] = useState(() => colorOptions.map(() => new Animated.Value(1)));

  const handlePress = (color: ColorBand, setter: (color: ColorBand) => void, selected: ColorBand, idx: number) => {
    setter(color);
    Animated.sequence([
      Animated.timing(scaleAnim[idx], { toValue: 1.15, duration: 120, useNativeDriver: true }),
  ...
  const renderColorButton = (
    ...
    <Animated.View key={color} style={{ transform: [{ scale: scaleAnim[idx] }] }}>
      <Pressable
        onPress={() => handlePress(color, setter, selected, idx)}
    ...
    return (
        <View className="flex-1 justify-start items-center pt-8 px-2 w-full bg-white">
            ...
            <View className="w-full max-w-xl px-2 py-4">
                <Text className="text-3xl md:text-5xl font-extrabold mb-2 text-center text-blue-900">Calculadora de Resistores (4 Faixas)</Text>
                ...
                <View className="flex-row flex-wrap justify-center items-start px-1 py-2 w-full gap-2">
                    ...
                    <View className="flex-col items-center mx-2">
                      <Text className="text-xs font-semibold mb-1 text-gray-600">Faixa 1</Text>
                        {colorOptions.slice(0, 10).map((color, idx) => renderColorButton(color, setBand1, band1, idx))}
                    </View>
                    {/* Faixa 2 */}
                    <View className="flex-col items-center mx-2">
                      <Text className="text-xs font-semibold mb-1 text-gray-600">Faixa 2</Text>
                        {colorOptions.slice(0, 10).map((color, idx) => renderColorButton(color, setBand2, band2, idx))}
                    </View>
                    {/*Multiplicador */}
                    <View className="flex-col items-center mx-2">
                      <Text className="text-xs font-semibold mb-1 text-gray-600">Multiplicador</Text>
                        {colorOptions.map((color, idx) => renderColorButton(color, setMultiplier, multiplier, idx))}
                    </View>
                    {/* Tolerância */}
                    <View className="flex-col items-center mx-2">
                      <Text className="text-xs font-semibold mb-1 text-gray-600">Tolerância</Text>
                        {colorOptions.map((color, idx) => renderColorButton(color, setTolerance, tolerance, idx))}
                    </View>
                </View>
            </View>
        </View>
